      ...
      <div className="w-full min-h-screen bg-[#F9FAFB]">
        <div className="px-[3.8%] py-6">
          ...
          <div className="flex gap-6 flex-wrap lg:flex-nowrap">
            ...
            >
              {/* Table Header */}
              <div className="grid" style={{ gridTemplateColumns: '0.6fr 2fr 1fr 1.5fr 0.8fr 0.6fr', padding: '12px 24px', borderBottom: '1px solid #E5E7EB', backgroundColor: '#F9FAFB' }}>
                <span></span>
              ...
              {inboundProducts.length === 0 ? (
                ...
                inboundProducts.map((product, index) => (
                  <div key={product.id} className="grid items-center" style={{ gridTemplateColumns: '0.6fr 2fr 1fr 1.5fr 0.8fr 0.6fr', padding: '16px 24px', borderBottom: index < inboundProducts.length - 1 ? '1px solid #E5E7EB' : 'none', backgroundColor: product.jtlProductId ? 'transparent' : '#FEF3C7' }}>
                    ...
                    <input type="number" min="1" step="1" value={product.qty} onChange={(e) => { const value = e.target.value; const newQty = value === '' ? 0 : parseInt(value); setInboundProducts(inboundProducts.map(p => p.id === product.id ? { ...p, qty: newQty } : p)); }} onBlur={(e) => { const value = parseInt(e.target.value); if (!value || value < 1) { setInboundProducts(inboundProducts.map(p => p.id === product.id ? { ...p, qty: 1 } : p)); } }} style={{ width: '60px', height: '32px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #D1D5DB', fontFamily: 'Inter, sans-serif', fontWeight: 400, fontSize: '14px', lineHeight: '20px', color: '#111827', textAlign: 'center' }} />
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {product.jtlProductId ? (
                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '2px 8px', borderRadius: '10px', backgroundColor: '#D1FAE5', color: '#059669', fontFamily: 'Inter, sans-serif', fontWeight: 500, fontSize: '11px' }} title={`JTL ID: ${product.jtlProductId}`}>Synced</span>
                      ) : (
                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '2px 8px', borderRadius: '10px', backgroundColor: '#FEF3C7', color: '#D97706', fontFamily: 'Inter, sans-serif', fontWeight: 500, fontSize: '11px' }} title="Product not synced with JTL FFN">Not synced</span>
                      )}
                    </div>
                  </div>
                ))
            ...
            {showProductList && productSearchQuery && filteredAvailableProducts.length > 0 && (
              <div style={{ width: '100%', maxWidth: '927px', borderRadius: '8px', border: '1px solid #E5E7EB', backgroundColor: '#FFFFFF', boxShadow: '0px 1px 2px 0px rgba(0, 0, 0, 0.06), 0px 1px 3px 0px rgba(0, 0, 0, 0.1)', overflow: 'hidden' }}>
                ...
                {productsLoading ? (
                  <div style={{ padding: '24px', textAlign: 'center', color: '#6B7280' }}>{tCommon('loading')}...</div>
                  ...
                  <div key={product.id} className="grid items-center" style={{ gridTemplateColumns: '2fr 1fr 1fr 0.6fr 0.8fr 0.6fr', padding: '16px 24px', borderBottom: index < filteredAvailableProducts.length - 1 ? '1px solid #E5E7EB' : 'none', backgroundColor: (product as any).jtlProductId ? 'transparent' : '#FFFBEB' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      {(product as any).jtlProductId ? (
                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '2px 8px', borderRadius: '10px', backgroundColor: '#D1FAE5', color: '#059669', fontFamily: 'Inter, sans-serif', fontWeight: 500, fontSize: '11px' }} title={`JTL ID: ${(product as any).jtlProductId}`}>Synced</span>
                      ) : (
                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '2px 8px', borderRadius: '10px', backgroundColor: '#FEF3C7', color: '#D97706', fontFamily: 'Inter, sans-serif', fontWeight: 500, fontSize: '11px' }} title="Product not synced with JTL FFN">Not synced</span>
                      )}
                    </div>
            ...
            <div style={{ width: '100%', maxWidth: '927px', borderRadius: '8px', gap: '20px', padding: '24px', backgroundColor: '#FFFFFF', boxShadow: '0px 1px 2px 0px rgba(0, 0, 0, 0.06), 0px 1px 3px 0px rgba(0, 0, 0, 0.1)' }}>
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <input type="text" value={eta} onChange={(e) => setEta(e.target.value)} placeholder="DD.MM.YYYY" maxLength={10} style={{ width: '100%', height: '38px', borderRadius: '6px', border: eta && !validateDateFormat(eta) ? '1px solid #EF4444' : '1px solid #D1D5DB', padding: '9px 13px', fontFamily: 'Inter, sans-serif', fontSize: '14px', lineHeight: '20px', color: '#111827', backgroundColor: '#FFFFFF', boxShadow: '0px 1px 2px 0px rgba(0, 0, 0, 0.05)' }} />
                  <span style={{ fontSize: '12px', color: '#6B7280', marginTop: '4px', display: 'block' }}>{tMessages('dateFormatHint') || 'Format: DD.MM.YYYY (e.g., 15.03.2024)'}</span>
                </div>
                <div>
            ...
            {hasProductsWithoutJtlId && (
              <div style={{ width: '100%', maxWidth: '927px', padding: '12px 16px', borderRadius: '8px', backgroundColor: '#FEF3C7', border: '1px solid #F59E0B', fontFamily: 'Inter, sans-serif', fontSize: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                  ...
                  <div>
                    <p style={{ color: '#92400E', fontWeight: 500, margin: 0 }}>{tMessages('jtlWarningTitle') || 'Some products are not synced with JTL FFN'}</p>
                    <p style={{ color: '#B45309', margin: '4px 0 0 0', fontSize: '13px' }}>{tMessages('jtlWarningDescription') || `The following products will NOT be included in the JTL inbound: ${productsWithoutJtlId.map(p => p.sku).join(', ')}. Please sync these products with JTL FFN first.`}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {validationErrors.length > 0 && (
              <div style={{ width: '100%', maxWidth: '927px', padding: '12px 16px', borderRadius: '8px', backgroundColor: '#FEE2E2', border: '1px solid #EF4444', fontFamily: 'Inter, sans-serif', fontSize: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                  ...
                  <div>
                    <p style={{ color: '#991B1B', fontWeight: 500, margin: 0 }}>{tMessages('validationErrors') || 'Please fix the following errors:'}</p>
